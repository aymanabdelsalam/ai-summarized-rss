...
def summarize_text_with_gemini(text_to_summarize, article_title="this article"):
    ...
    try:
        prompt = (
            f" اعمل ملخص شامل بدون الاخلال عن مقصد ومحتوى المقاله الأصلى من كذا فقرة للمقالة الإخبارية دي بعنوان '{article_title}' باللهجة المصرية  العامية بدون الإشارة للمقالة الأصليه وكأنك بتكتب انت مقاله جديدة. "
            f"الملخص المفروض يغطي النقط الأساسية، الحجج المهمة، وأي استنتاجات ضرورية ممكن تظن أن القارئ محتاجها لفهم المقاله. "
            f"الهدف إنك تكتب مقالة ملخصة من عدة فقرات، على الأقل اربع فقرات، وتقدر تزود أكتر عشان تشرح المقالة كويس وتوصل المطلوب بطريقة مبسطة وفي وقت قليل."
            f"ابدأ المقالة بتحية مصرية سهله، زي مساء الفل أو صباح الفل حسب توقيت اعداد التلخيص بدون استخدام كلمة يا جدعان او توجيه التحية لمصر او أهل مصر."
